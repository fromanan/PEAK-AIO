...
public class PeakMod : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        gameObject.AddComponent<EventComponent>();
    }
    ...
    private bool DrawSearchableCombo(
        string label,
        ref int selectedIndex,
        List<string> items,
        ref string searchBuffer)
    {
        var changed = false;

        // Draw input field
        var inputId = $"Search##{label}";
                ...
                switch (selectedTab)
                {
                    ...
                    case 1:
                    {
                        ...
                        if (ImGui.CollapsingHeader("Teleport##PlayerTeleport", ImGuiTreeNodeFlags.DefaultOpen))
                        {
                            ...
                            {
                                Logger.LogInfo(
                                    $"[Teleport] Requested to X:{Globals.TeleportX} Y:{Globals.TeleportY} Z:{Globals.TeleportZ}");
                        ...
                        if (ImGui.CollapsingHeader("Details", ImGuiTreeNodeFlags.DefaultOpen))
                        {
                            if (ConfigManager.JumpMod.Value)
                            {
                                ...
                                DrawSliderFloat(ConfigManager.JumpAmount, "##jump_amt", 10.0f, 500.0f,
                                    "Jump Mult: %.2f");
                            }
                            ...
                            {
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(ConfigManager.SpeedAmount, "##speed_amt", 1.0f, 20.0f,
                                    "Move Speed: %.2f");
                            }
                            ...
                            {
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(ConfigManager.ClimbAmount, "##climb_amt", 1.0f, 20.0f,
                                    "Climb Speed: %.2f");
                            }
                            ...
                            {
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(ConfigManager.VineClimbAmount, "##vine_climb_amt", 1.0f, 20.0f,
                                    "Vine Speed: %.2f");
                            }
                            ...
                            {
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(ConfigManager.RopeClimbAmount, "##rope_climb_amt", 1.0f, 20.0f,
                                    "Rope Speed: %.2f");
                            }
                            ...
                            if (ConfigManager.FlyMod.Value)
                            {
                                ...
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(ConfigManager.FlyAcceleration, "##fly_acceleration", 10f, 300f,
                                    "Fly Acceleration: %.2f");
                            }
                        }
                    ...
                    case 2:
                    {
                        if (Globals.ItemNames.Count is 0)
                        {
                            Utilities.UpdateItems();
                        }

                        var assignQueue = new List<(int slot, int itemIndex)>();
                        ...
                        if (ImGui.BeginTable("InventorySlots", 3, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg))
                        {
                            ...
                            for (int slot = 0; slot < 3; slot++)
                            {
                                ...
                                if (Player.localPlayer?.itemSlots is not null &&
                                    ...
                                    Player.localPlayer.itemSlots[slot]?.prefab is not null)
                                {
                                ...
                                if (DrawSearchableCombo($"##Combo{slot}", ref selected, Globals.ItemNames,
                                        ref Globals.ItemSearchBuffers[slot]))
                                {
                                ...
                                ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 4);
                                DrawSliderFloat(rechargeAmountConfig, $"##recharge_mount##{slot}", 0f, 100f,
                                    "Charge: %.1f");
                        ...
                        foreach ((int slot, int itemIndex) in assignQueue)
                        {
                    ...
                    case 3:
                    {
                        ...
                        if (Globals.AllPlayers.Count is 0)
                        {
                    ...
                    case 4:
                    {
                        ...
                        if (ImGui.CollapsingHeader("Actions", ImGuiTreeNodeFlags.DefaultOpen))
                        {
                            if (Globals.SelectedLuggageIndex >= 0 && Globals.SelectedLuggageIndex < Globals.LuggageLabels.Count)
                            {
                                ...
                                {
                                    Logger.LogInfo(
                                        $"[UI] Warp requested for index {Globals.SelectedLuggageIndex} - {label}");
                                    Vector3 luggageCoords =
                                        Globals.LuggageObjects[Globals.SelectedLuggageIndex].Center();
